...
\begin{frame}{My application: ideal implementation}
  \begin{itemize}
  \item I2C EEPROM on each camera (fixed slave address)
  \item Detect link, read model in EEPROM, load DT overlay
  \item A DT overlay for each model
  \item Similar to BBB capes, RPi hats, but hot-plug
  \item Problems:
    \begin{itemize}
    \item Dynamic DT overlays not yet in mainline
    \item Bidirectional endpoint connections
    \item V4L2 limitation (covered later)
    \end{itemize}
  \end{itemize}
\end{frame}
