...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% TODO: Agenda
%% TODO stile riquardi? https://tex.stackexchange.com/questions/112559/box-around-minted-environment
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{frame}{Motivation}
  \begin{itemize}
  \item Two different tools, similar goals
  \item People from each community have a wrong idea about the other
    tool
  \item Goal: tell them how the daily tasks look like ``on the other
    side''
  \end{itemize}
\end{frame}
...
\begin{frame}{Similar\dots}
  ...
  In a nutshell: a dependency graph with actions to build each node.
\end{frame}
...
%%%% ***NOTE***: Focus of this talk: embedded system, root filesystem generation
...
%%%%  [????? move ``layers can create problems'' later, after introducing Layers???? but where??? ]
\bg{yocto}
\begin{frame}{Ingredients}
  \begin{enumerate}
  ...
  \item Additional layers can be useful
    ...
    \end{itemize}
  \item Layers can create problems
    \begin{itemize}
    ...
    \item Solution: fix the offending code bypass ({\tt .bbappend}) or disable it ({\tt PNBLACKLIST}) and provide an alternative
    \item Extreme solution: don't use the layer, copy only what you
      need
    \end{itemize}
  \end{enumerate}
\end{frame}
...
\begin{frame}
  \center\includegraphics[height=1.0\textheight]{images/br-menuconfig-kernel.png}
\end{frame}
...
\begin{frame}[fragile]{Configure}
  ...
  \item Add layers to {\tt conf/bblayers.conf}
  \end{itemize}

  \begin{minted}[bgcolor=codebackground,frame=single,autogobble,fontsize=\small]{diff}
   BBLAYERS ?= " \
...
\begin{frame}{Configure}
  ...
  \item Smooth start: find a defconfig for a similar board
    ...
    \end{itemize}
  \item Later, add your top-level layer
    \begin{itemize}
...
\begin{frame}{Buildroot packages}
  ???? change: have 1 slide for package/recipe side-by-side, one for step/task side-by-side??????
  \begin{itemize}
  \item {\em package}: a single software with downloadable sources and
    rules for building
    \begin{itemize}
    \item Examples: binutils, busybox, gcc, libxml2, linux
    \item Each package is a Make target
    \item {\tt make libxml2}
    \item {\tt host-<PKG>}: the same package built for the development
      host (native build)
    \end{itemize}
  \item Each package requires several steps to be built
    \begin{itemize}
    ...
    \item {\tt make libxml2-configure busybox host-binutils-build}
    \end{itemize}
  \end{itemize}
\end{frame}

\begin{frame}{Buildroot package: default steps}
  \center\includegraphics[height=0.95\textheight]{images/br-steps.pdf}
\end{frame}

\begin{frame}{Yocto recipes}
  \begin{itemize}
  \item {\em recipe}: a single software with downloadable sources and
    rules for building
    \begin{itemize}
    \item Examples: binutils, busybox, gcc, libxml2
  ...
  \item Each package requires several steps to be built
    \begin{itemize}
    \item Called {\em tasks} (often prefixed with {\tt do\_})
    ...
    \item {\tt bitbake -c configure libxml2}
    \end{itemize}
  \end{itemize}
\end{frame}

\begin{frame}{Yocto recipe: default tasks}
...
\begin{frame}[fragile]{A simple Yocto package: the {\tt .bb} file}
  ...
  \begin{minted}[frame=single,autogobble,fontsize=\footnotesize]{bash}
    ...
    do_install() {
        install -D -m 0755 foo ${D}${bindir}/foo
    }
  \end{minted}
\end{frame}
...
\bg{both}
%%%% *****NOTE**** add slide with variables PN/PV/B/S/D vs
%%%%      BUILD\_DIR, TARGET\_DIR ETC??? and say {\tt install} installs in
%%%%      a temp dir VS the target rootfs area????
  ...
  \item To extend them define hooks
    ...
    \item {\tt FOO\_POST\_<STEP>\_HOOKS}
      \begin{minted}[autogobble,fontsize=\footnotesize]{makefile}
        ...
        endef
        FOO_POST_PATCH_HOOKS += FOO_CREATE_CONF_FILE
      \end{minted}
    \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile]{Buildroot: concise output}
  ...
  \item The output of each step is discarded
  \item Failure? Run plain {\tt make} to see the error messages
  \end{itemize}
\end{frame}
...
\begin{frame}{Yocto: inspect build logs}
  ...
  \item For each task a log file is saved
    \begin{itemize}
    \item in {\tt tmp/work/<ARCH>/<RECIPE>/<VERSION>/temp/log.do\_<TASK>}
...
\bg{br}
\begin{frame}{Buildroot: the power of Make}
  \begin{itemize}
  \item Buildroot uses plain Make to build
  \item Call {\tt make -qp} to print the whole Make database
    \begin{itemize}
    \item Variables and the file where they were set
...
\begin{frame}{Buildroot dependency tracking: stamp files}
  \begin{itemize}
  \item The standard Make dependency tracking
    (program \textrightarrow{} .o \textrightarrow{} .c)
    is not doable in a buildsystem
...
\begin{frame}{Yocto: {\tt .bb} files}
  ...
  \item A common layout:
    \begin{itemize}
    \item Build options, toolchain, {\tt MACHINE}: {\tt
...
\begin{frame}[fragile]{Yocto: adding packages}
  ...
  \item Create an image recipe ({\tt <MYLAYER>/recipes-*/images/*-image-*.bb})
    \begin{minted}[frame=single,autogobble,fontsize=\footnotesize]{bash}
      ...
      IMAGE_INSTALL += "htop packagegroup-media"
    \end{minted}
  \item Package groups ({\tt <MYLAYER>/recipes-*/packagegroups/packagegroup-*.bb})
    \begin{minted}[frame=single,autogobble,fontsize=\footnotesize]{bash}
...
\bg{yocto}
\begin{frame}{Yocto: layers and {\tt .bbappend}}
  \begin{itemize}
  \item The preferred way: layers
    \begin{itemize}
    \item {\tt <BASE\_LAYER>/recipes-xyz/foo/bar.bb}
    \item {\tt <MY\_LAYER>/recipes-xyz/foo/bar.bbappend}
    \end{itemize}
...
\begin{frame}{Buildroot: {\tt BR2\_EXTERNAL}}
  \begin{itemize}
  \item {\tt BR2\_EXTERNAL} is similar to Yocto layers, but simpler
    \begin{itemize}
    \item You can add entire new packages, configs, etc
    \item Does generally not allow to change lower layer packages
    \end{itemize}
  \item You can set make variables
    \begin{itemize}
    \item In {\tt local.mk}
    \item In {\tt .mk} files in your {\tt BR2\_EXTERNAL} tree
    \item Works because the result is a large Makefile
    \item Discouraged
      \begin{itemize}
      \item Preferred: fork Buildroot, then submit your improvements upstream
      \end{itemize}
    \end{itemize}
  \end{itemize}
\end{frame}
...
\bg{yocto}
%%%%  [*** SIMILAR TO CLASSES EXAMPLE IN ``WRITING RECIPES'' - REMOVE ONE????]
